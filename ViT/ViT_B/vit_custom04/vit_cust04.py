...
Original file is located at
    https://colab.research.google.com/drive/14FNzcquiS10Q1-GpGHvPoZ1ahSsrgEJN

# ViT_Custom02

* Based on **ViT_4a**
* Modified Kaggle Dataset (6x29,400) [32, 32, 3]
* Validation metrics include:
  * Accuracy, Loss, Top5Accuracy
...
* train_lab_categorical  (50000, 10)
* test_lab_categorical  (10000, 10)
"""
...
print("train_lab_categorical ", train_lab_categorical.shape)
print("test_lab_categorical ", test_lab_categorical.shape)
...
train_data_batches = training_data.shuffle(buffer_size=112896).batch(128).prefetch(buffer_size=autotune)
valid_data_batches = validation_data.shuffle(buffer_size=28224).batch(32).prefetch(buffer_size=autotune)
test_data_batches = test_data.shuffle(buffer_size=28224).batch(32).prefetch(buffer_size=autotune)
...
2. **generate_patch_conv_orgPaper**: What was done by the google researchers. Number of filters in the Conv2D layer matches the hidden dimension i.e. query/key dimension in multi-head attention layer.
"""
...
"""## (SKIP) Visualise Image & Encoded Patches"""
...
Implement the Transformer Encoder block shown above:
...
train_ds = (training_data.shuffle(112896).batch(128).map(lambda x, y: (data_augmentation(x), y), num_parallel_calls=autotune).prefetch(autotune))
valid_ds = (validation_data.shuffle(28224).batch(32).prefetch(autotune))

"""## Build and Test ViT Model

Display the architecture of the model - comment out?
"""
...
"""## [SKIPPED] Print model to FLOW_DIAGRAM

Useful for report / thesis

## Model - Compile, Reduce & FIT

120 Epochs - Accuracy approx. 56% ?
"""

ViT_model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=2e-3), 
                  loss = tf.keras.losses.CategoricalCrossentropy(from_logits=True), 
                  metrics=[tf.keras.metrics.CategoricalAccuracy(name="accuracy"), 
                           tf.keras.metrics.TopKCategoricalAccuracy(k=5, name='top5 acc')])
...
ViT_Train = ViT_model.fit(train_ds, epochs = 120, 
...
Pkl_Filename = "ViT_Custom02.pkl"  
with open(Pkl_Filename, 'wb') as file:  
    pickle.dump(ViT_Train, file)
